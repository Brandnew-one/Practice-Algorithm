...
var d: [Int] = .init(repeating: maxValue, count: 100002)
var heap = Heap<Edge>(sortFunction: <)

d[n] = 0
heap.insert(node: Edge(destination: n, cost: 0))
while !heap.isEmpty {
  let cur = heap.remove()!
  ...
  for i in 1...3 {
    var next: Edge
    if i == 1 {
      if cur.destination + 1 >= 100001 { continue }
      next = .init(destination: cur.destination + 1, cost: 1)
    } else if i == 2 {
      if cur.destination - 1 < 0 { continue }
      next = .init(destination: cur.destination - 1, cost: 1)
    } else {
      if cur.destination * 2 >= 100001 { continue }
      next = .init(destination: cur.destination * 2, cost: 0)
    }
