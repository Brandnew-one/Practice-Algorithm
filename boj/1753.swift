...
var d: [Int] = .init(repeating: .max, count: v + 2)
var adj: [[Edge]] = .init(repeating: [], count: v + 2)

for _ in 1...e {
  let input = readLine()!.split(separator: " ").map { Int($0)! }
  let edge = Edge(destination: input[1], cost: input[2])
  if let index = adj[input[0]].firstIndex(where: { $0.destination == edge.destination }){
    if adj[input[0]][index].cost > edge.cost {
      adj[input[0]].remove(at: index)
      adj[input[0]].append(.init(destination: input[1], cost: input[2]))
    }
  } else {
    adj[input[0]].append(.init(destination: input[1], cost: input[2]))
  }
...
d[k] = 0
heap.insert(node: .init(destination: k, cost: 0))
